...
c       jflag= 1                          !mean errors
          call Gaia_errors(xmonth,xCAfactor,jflag,V,VI,a,ao,ae,
     &             p,po,pe,ap,apo,ape,GRVS,Llavor)
c---------------------------------------------------------
...
c	call gal_mu(distobs,globs,gbobs,Uobs,Vobs,Wobs,xmubo,xmulso
c     &              ,vro)

c        call pmeq_to_pmgal(pmra,pmdec,ra,dec,pml,pmb)
        xmira=a(1)
        xmidec=a(2)
        xmipmra=a(4)
        xmipmdec=a(5)
        call pmeq_to_pmgal(xmipmra,xmipmdec,xmira,xmidec,xmuls,xmub)
        xmirao=ao(1)
        xmideco=ao(2)
        xmipmrao=ao(4)
        xmipmdeco=ao(5)
        call pmeq_to_pmgal(xmipmrao,xmipmdeco,xmirao,xmideco,
     &                     xmulso,xmubo)
...
c Computation of (U,V,W) without errors
         ...
         sigvrad=AE(6)
         remu=abs(sigmu/xmub)
         reD=(distobs-dist)/dist
        
         if (Gmag.le.20.0) then

         WRITE(NCO,100)Av,V,Gmag,GRVS,relerr,X,Y,Z,VX,VY,VZ,
     &              xpar,gl/deg,gb/deg,dist,xmuls,
     &              xmub,vrad,Xobs,Yobs,Zobs,VXobs,VYobs,VZobs
     &              ,xparobs,globs/deg,gbobs/deg,distobs
     &              ,xmulso,xmubo,
     &              vrob,reD,sigmu,sigvrad,VI,remu,revrad
         endif
 100     FORMAT(37(F18.10,1X))
         GOTO 10
 111      CLOSE(NCI)
        CLOSE(nco)
